...
class Renderer(nn.Module):
    def __init__(self, opt):
        ...
        # GUI 모드이면서 (또는 Windows일 때)만 GL 컨텍스트, 그 외는 CUDA 전용
        # self.opt.force_cuda_rast 는 “무조건 CUDA만 쓰라”는 플래그임
        # opt.gui=False(헤드리스) 상태에서 not self.opt.gui 가 True 가 되므로, 이 gpu 서버 환경인데도 불구하고 RasterizeGLContext() 를 선택해버림.
        # 화면이 없는 환경에서 EGL 초기화(eglInitialize())나 CUDA-GL interop(cudaGraphicsGLRegisterBuffer)이 실패. 즉, GUI 화면을 못 띄워서 발생하는 문제
        if not self.opt.force_cuda_rast and (self.opt.gui or os.name == 'nt'):
